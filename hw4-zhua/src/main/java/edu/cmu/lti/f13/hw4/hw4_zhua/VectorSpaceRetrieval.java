 ...
 * 
 */
package edu.cmu.lti.f13.hw4.hw4_zhua;
...
public class VectorSpaceRetrieval {
	...
	public static void main(String [] args) 
	   ...
	   if (descUrl == null) {
		...
		URL stopUrl = VectorSpaceRetrieval.class.getResource("/stopwords.txt");
		if (stopUrl == null){
		  throw new IllegalArgumentException("Error opening stopwords.txt");
		}
		stopwords = new HashSet<String>();
		BufferedReader mbr = new BufferedReader(new InputStreamReader(stopUrl.openStream()));
		while ((sLine = mbr.readLine()) != null) {
		  stopwords.add(sLine);
		}
		mbr.close();
